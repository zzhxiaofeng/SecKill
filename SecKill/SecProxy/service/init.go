...
func InitService(serviceConf *SecSkillConf) (err error) {
	...
	initRedisProcessFunc()//初始化redis的处理函数
...
func SyncIpBlackList() {
	...
	for {
		...
		if len(ipList) > 100 || curTime-lastTime > 5 {//当ip达到100个或者时间过了五秒，将ip放入ipBlackMap
...
func SyncIdBlackList() {
	...
	for {
		...
		if len(idList) > 100 || curTime-lastTime > 5 {//当ip达到100个或者时间过了五秒，将ip放入ipBlackMap
			secKillConf.RWBlackLock.Lock()
			for _, v := range idList {
