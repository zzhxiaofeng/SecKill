...
func initConfig(confType, filename string) (err error) {
	...
	appConfig.Proxy2LayerRedis.RedisQueueName = conf.String("redis::resis_proxy2layer_queue_name")//配置proxy2layer存放数据的队列的名字
	...
	appConfig.Layer2ProxyRedis.RedisQueueName = conf.String("redis::redis_layer2proxy_queue_name")//配置layer2proxy存放数据的队列的名字
	if len(appConfig.Proxy2LayerRedis.RedisQueueName) == 0 {
